...
struct pokemon *cargar_nombre_pokemon(char linea[MAX_LINEA]) {
  ...
  if (nuevo_pokemon == NULL) return NULL;
...
void leer_pokemones(FILE *archivo, struct info_pokemon *ip) {
  ...
  while (fgets(linea, 200, archivo) != NULL && !error) {
    if (linea_recorrida == 0) {
      struct pokemon **aux =
          realloc(ip->pokemones, ((unsigned int)ip->cantidad_pokemones + 1) *
      ...
      if (aux == NULL) {
        ...
        if (ip->pokemones[ip->cantidad_pokemones] == NULL) {
          ...
          if (aux2 == NULL) {
            free(ip->pokemones[ip->cantidad_pokemones]);
            error = true;
          } else {
      ...
      ip->pokemones[ip->cantidad_pokemones]
          ->ataques[ip->pokemones[ip->cantidad_pokemones]->cantidad_ataques] =
...
pokemon_t *pokemon_buscar(informacion_pokemon_t *ip, const char *nombre) {
  if (ip == NULL || nombre == NULL) return NULL;
...
int con_cada_ataque(pokemon_t *pokemon,
  ...
  if (pokemon == NULL || f == NULL) return 0;
